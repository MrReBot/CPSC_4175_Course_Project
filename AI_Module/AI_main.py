...
def readDynamicDatabase(self):
    fileDictionary = {}
    courselist= Parser.parse_txt("DynamicElectives_Database.txt",db)
...
def createCompElectivesText(self):
    if(exists("DefaultElectives_Database.txt") == False):
        file =open("DefaultElectives_Database.txt", "a")
        for course in db.get_tag("CPSC-ELECT"):
            file.write(course +"\~n")
                
#Method to choose courses from the dynamic database
def chooseFromDynamicDatabase(self,courseList, numberOfElectives):
    currentChosenCourse = ""
...
def chooseFromDefaultDatabase(self, numberOfElectives):
    chosenCourse = ""
    electiveList = []
    #Save the text file as a list of courses
    courseList= Parser.parse_txt("DefaultElectives_Database.txt",db)
...
def chooseElectives(self, majorName, number_of_electives):
    fileDictionary = {}
    electiveList = []
    fileDictionary =self.readDynamicDatabase()
    ...
    if majorName in fileDictionary:
        electiveList = self.chooseFromDynamicDatabase(fileDictionary[majorName],number_of_electives)
    #else choose all randomly from the default
    else:
        electiveList = self.chooseFromDefaultDatabase(number_of_electives)
    return electiveList;
...
def saveToDynamicDatabase(self, majorName, className):
    fileDictionary = {}
    fileDictionary =self.readDynamicDatabase()
    ...
    file = open("DynamicElectives_Database.txt",'w')
...
def deleteDynamicData(self):
    open('DynamicElectives_Database.txt', 'w').close()
